  ...
  req.on('end', function() {
    ...
    if (parts.file) {
      fileInfo = {
        ...
        first50: parts.file.data.slice(0, 50).toString('utf8')
      };
    }

    var userId = parts.fields.userId || 'none';
    var apiKey = parts.fields.apiKey ? 'yes(' + parts.fields.apiKey.length + ')' : 'none';
    ...
    res.status(200).json({
      ...
      fields: {
        userId: userId,
        apiKey: apiKey,
        assetType: parts.fields.assetType || 'none',
        displayName: parts.fields.displayName || 'none'
      },
      file: fileInfo,
      mime: mime
    });
